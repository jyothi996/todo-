...
app.get("/todos/:todoId", async (request, response) => {
  ...
  const todoQuery = await database.get(getTodo);
  response.send(outPutResult(todoQuery));
});
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    console.log(newDate);
    const getNewDate = `SELECT * FROM todo WHERE due_date = '${newDate}';`;
    const result = await database.all(getNewDate);
    ...
    if (status === "TO DO" || status === "IN PROGRESS" || status === "DONE") {
      if (
        ...
        VALUES(
          ...
          const result = await database.run(newDateQuery);
          //console.log(result);
